...
def run_comprehensive_ablation(data_path="data/processed/featured_data.csv", advanced_model_path="reports/advanced/best_model.pth"):
    ...
    if os.path.exists(advanced_model_path):
        ...
        model_c.eval()
        with torch.no_grad():
            preds_c_scaled = model_c(torch.FloatTensor(X_test), torch.FloatTensor(R_test)).numpy()
    ...
    if os.path.exists(advanced_model_path):
        ...
        for nl in noise_levels:
            ...
            with torch.no_grad():
                p_noisy = model_c(torch.FloatTensor(X_noisy), torch.FloatTensor(R_test)).numpy()
