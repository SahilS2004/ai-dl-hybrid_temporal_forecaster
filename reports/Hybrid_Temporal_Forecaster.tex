...
\subsubsection{Stage 1: Probabilistic Regime Extraction}

A Gaussian Mixture Model (GMM) is trained on rolling volatility features $\{\sigma_{24h}, \mu_{24h}\}$. Unlike hard-clustering methods like K-Means, the GMM provides posterior probabilities $P(\text{Regime} = k \mid \mathbf{x}_t)$, capturing the inherent uncertainty in regime transitions.
...
The novel \textbf{RGaA} block modulates the standard multi-head self-attention weights. A secondary gating network $\mathcal{G}(\cdot)$ takes the GMM probabilities $\mathbf{p}_t$ and outputs a scaling vector:
\begin{equation}
\mathbf{h}_t' = \text{Attention}(\mathbf{Q}, \mathbf{K}, \mathbf{V}) \odot \sigma(\mathbf{W}_g \mathbf{p}_t + \mathbf{b}_g)
\end{equation}
where $\odot$ denotes the Hadamard product. This allows the model to dynamically "gate" the information flow: during extreme regimes, the gate can amplify recent lag dependencies while dampening seasonal ones.
